...
<nav
  class="bg-white border-gray-200 dark:bg-gray-900 dark:border-gray-700 text-sm antialiased">
  <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
    <a href="{{ url()->current() }}"
      class="flex items-center space-x-3 rtl:space-x-reverse">
      <img src="https://flowbite.com/docs/images/logo.svg" class="h-8 mr-5"
        alt="Flowbite Logo" />
    </a>
      ...
      <ul
        ...
        <li>
          ...
          <div id="preproDropdown"
            ...
            <ul class="py-2 text-gray-700 dark:text-gray-400"
              ...
              <x-nav-link-dropdown href="{{ route('preprocessing.index') }}">Data
                Preprosesing</x-nav-link-dropdown>

              @if ($show)
                <x-nav-link-dropdown href="{{ route('result.train-data') }}">Data
                  Train</x-nav-link-dropdown>
                <x-nav-link-dropdown href="{{ route('result.test-data') }}">Data
                  Test</x-nav-link-dropdown>
              @endif
        ...
        @if ($show)
          {{-- <x-nav-link href="https://sentiment-analysis-eta-drab.vercel.app">Predict
            Review</x-nav-link> --}}
          <x-nav-link href="{{ route('result.naive-bayes') }}">Naive Bayes
            Clsf.</x-nav-link>
          <x-nav-link href="{{ route('result.confusion-matrix') }}">Conf.
            Matrix</x-nav-link>
          ...
          <div id="userDropdown"
            ...
            <ul class="py-2 text-gray-700 dark:text-gray-400"
              ...
              @if (Auth::user()->role == 'admin')
                <x-nav-link-dropdown href="{{ route('manage-user.index') }}">Manage
                  Users</x-nav-link-dropdown>
              @endif

              <x-nav-link-dropdown href="{{ route('my-profile.index') }}">My
                Profile</x-nav-link-dropdown>
