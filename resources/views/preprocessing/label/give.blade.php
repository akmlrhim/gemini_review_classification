<x-layout>
  ...
  <div class="max-w-2xl mx-auto py-10 bg-white shadow-md rounded-lg p-6 mb-8 border border-gray-200">
    <form action="{{ route('preprocessing.label.update', $label->id) }}" method="POST">
      ...
      <div class="mb-4">
        <label for="label" class="block text-sm font-medium text-gray-700 mb-3">"{{ $label->case_folding }}"</label>
      ...
      <div>
        ...
        <button type="submit" class="bg-blue-500 text-white text-sm px-2 py-1 rounded-md">simpan</button>
      </div>
    </form>
  </div>
</x-layout>
