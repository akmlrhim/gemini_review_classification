<x-layout>
  ...
  <div class="max-w-2xl mx-auto py-10 bg-white shadow-md rounded-lg p-6 mb-8 border border-gray-200">
    <form action="{{ route('preprocessing.label.update', $label->id) }}" method="POST">
      ...
      <div class="mb-4">
        <label for="label" class="block text-md font-medium text-gray-700 mb-3">komentar : {{ $label->case_folding }}</label>
        <select id="label" name="label"
          ...
          <option value="netral" {{ $label->label == 'netral' ? 'selected' : '' }}>netral</option>
      ...
      <div>
        <button type="button" onclick="window.location.href='{{ route('preprocessing.label') }}'"
          class="bg-gray-500 text-white px-2 py-1 rounded-md">kembali</button>
        <button type="submit" class="bg-blue-500 text-white px-2 py-1 rounded-md">simpan</button>
      </div>
    </form>
  </div>
</x-layout>
