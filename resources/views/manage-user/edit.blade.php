<x-layout>
  ...
  <div
    ...
    <form action="{{ route('manage-user.update', $user->id) }}" class="space-y-4"
      method="POST">
      @csrf
      @method('PUT')
      <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
        ...
        <div class="flex-1">
          <select name="role" id="role"
            class="w-full font-medium bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500">
            <option value="" disabled
              {{ old('role', $user->role ?? '') ? '' : 'selected' }}>choose a role
            </option>
            <option value="admin"
      ...
      <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
        ...
        <div class="flex gap-2">
          ...
          <button type="submit"
            class="bg-blue-500 text-sm text-white px-3 py-1 rounded-md uppercase">Save</button>
        </div>
      </div>
    </form>
  </div>
