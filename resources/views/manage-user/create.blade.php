  ...
  <div class="max-w-5xl mx-auto py-10 bg-white shadow-md rounded-lg p-6 mb-8 border border-gray-200">
    <form action="{{ route('manage-user.store') }}" class="space-y-4" method="POST">
      ...
      <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
        <label for="email" class="w-full md:w-40 text-sm font-medium text-gray-900 capitalize">Email</label>
        <div class="flex-1">
          <input type="email" name="email" id="email"
            class="w-full text-sm bg-gray-50 border border-gray-300 text-gray-900 rounded-lg p-2 focus:ring-primary-600 focus:border-primary-600"
      ...
      <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
        <label for="name" class="w-full md:w-40 text-sm font-medium text-gray-900 capitalize">Name</label>
        <div class="flex-1">
      ...
      <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
        <label for="password" class="w-full md:w-40 text-sm font-medium text-gray-900 capitalize">Password</label>
        <div class="flex-1">
          <input type="password" name="password" id="password"
            class="w-full text-sm bg-gray-50 border border-gray-300 text-gray-900 rounded-lg p-2 focus:ring-primary-600 focus:border-primary-600"
            placeholder="masukkan password" autocomplete="off" value="{{ old('password') }}">
          @error('password')
            <small class="text-red-500">{{ $message }}</small>
          @enderror
        </div>
